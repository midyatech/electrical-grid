...
function ExpandTreeNode($currentNodeSpan)
{
	...
	if (children.is(":visible")) {
	    children.hide('fast');
	    $currentNodeSpan.attr('title', 'Expand this branch').find(' > i').addClass('icon-plus-sign').removeClass('icon-minus-sign');
	} else {
	    children.show('fast');
	    $currentNodeSpan.attr('title', 'Collapse this branch').find(' > i').addClass('icon-minus-sign').removeClass('icon-plus-sign');
	}
	//e.stopPropagation();
}
