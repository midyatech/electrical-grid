...
if ($_GET["id"]) {
    ...
    $meter = $sc->GetMeterDetails($id);
    $trace = $sc->GetMeterTrace($id);

    $e_trace = null;
    if ($meter) {
        $enclosure_id = $meter[0]["enclosure_id"];
        if ($enclosure_id != null) {
            $e_trace = $sc->GetEnclosureTrace($enclosure_id);
        }
    }

    $ml = $sc->GetMeterLinking($id);
}
...
$html->OpenDiv("row");
{
    $html->OpenSpan(12);
    {
        ...
        $html->OpenWidget ("meter_details", $actions, array('collapse' => false, 'fullscreen'=>false));
        {
            $html->OpenForm ( "", "form2", "horizental");
            {
                $html->OpenDiv("row");
                {
                    $html->OpenSpan(6);
                    {
                        $html->DrawFormField("label", "meter_sn", $meter[0]["meter_sn"], NULL, array("class"=>"form-control", "flow"=>"horizental") );
                    }
                    $html->CloseSpan();
                    $html->OpenSpan(6);
                    {
                        $html->DrawFormField("label", "Model", $meter[0]["Model"], NULL, array("class"=>"form-control", "flow"=>"horizental") );
                    }
                    $html->CloseSpan();
                    $html->OpenSpan(6);
                    {
                        $html->DrawFormField("label", "Plant_No", $meter[0]["Plant_No"], NULL, array("class"=>"form-control", "flow"=>"horizental") );
                    }
                    $html->CloseSpan();
                    $html->OpenSpan(6);
                    {
                        $html->DrawFormField("label", "Serial_No", $meter[0]["Serial_No"], NULL, array("class"=>"form-control", "flow"=>"horizental") );
                    }
                    $html->CloseSpan();
                }
                $html->CloseDiv();
        ...
        $html->OpenWidget ("meter_linking", $actions, array('collapse' => false, 'fullscreen'=>false));
        {
        ...
        $html->OpenWidget ("meter_trace", null, array('collapse' => false, 'fullscreen'=>false));
        {
            print '<div class="timeline">';
            for ($i=0; $i < count($trace); $i++) {
                switch($trace[$i]["meter_trace_status_id"])
                {
                    case 1:
                        $icon = 'fa fa-plus';
                    ...
                    <div class="timeline-body">
                        ...
                        <div class="timeline-body-head">
                            <div class="timeline-body-head-caption">
                                <span class="timeline-body-alerttitle font-red-intense"><?php print $trace[$i]["meter_trace_status"];?></span>
        ...
        $html->OpenWidget ("consumer_info", $actions, array('collapse' => false, 'fullscreen'=>false));
        {
