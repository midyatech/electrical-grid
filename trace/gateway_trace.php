...
if ($_GET["id"]) {
    ...
    $gateway = $sc->GetGatewayDetails($id);
    $trace = $sc->GetGatewayTrace($id);

    $e_trace = null;
    if ($gateway) {
        $enclosure_id = $gateway[0]["enclosure_id"];
        if ($enclosure_id != null) {
            $e_trace = $sc->GetEnclosureTrace($enclosure_id);
        }
    }

    $ml = $sc->GetGatewayLinking($id);
}
...
$html->OpenDiv("row");
{
    $html->OpenSpan(12);
    {
        ...
        $html->OpenWidget ("gateway_details", null, array('collapse' => false, 'fullscreen'=>false));
        {
            $html->OpenForm ( "", "form2", "horizental");
            {
                $html->OpenDiv("row");
                {
                    $html->OpenSpan(6);
                    {
                        $html->DrawFormField("label", "gateway_sn", $gateway[0]["gateway_sn"], NULL, array("class"=>"form-control", "flow"=>"horizental") );
                    }
                    $html->CloseSpan();
                    $html->OpenSpan(6);
                    {
                        $html->DrawFormField("label", "Model", $gateway[0]["Model"], NULL, array("class"=>"form-control", "flow"=>"horizental") );
                    }
                    $html->CloseSpan();
                    $html->OpenSpan(6);
                    {
                        $html->DrawFormField("label", "ICCID", $gateway[0]["ICCID"], NULL, array("class"=>"form-control", "flow"=>"horizental") );
                    }
                    $html->CloseSpan();
                    $html->OpenSpan(6);
                    {
                        $html->DrawFormField("label", "Serial_No", $gateway[0]["Serial_No"], NULL, array("class"=>"form-control", "flow"=>"horizental") );
                    }
                    $html->CloseSpan();
                    $html->OpenSpan(6);
                    {
                        $html->DrawFormField("label", "IMEI", $gateway[0]["IMEI"], NULL, array("class"=>"form-control", "flow"=>"horizental") );
                    }
                    $html->CloseSpan();
                    $html->OpenSpan(6);
                    {
                        $html->DrawFormField("label", "ip_address", $gateway[0]["ip_address"], NULL, array("class"=>"form-control", "flow"=>"horizental") );
                    }
                    $html->CloseSpan();
                }
                $html->CloseDiv();
        ...
        $html->OpenWidget ("gateway_linking", null, array('collapse' => false, 'fullscreen'=>false));
        {
        ...
        $html->OpenWidget ("gateway_trace", null, array('collapse' => false, 'fullscreen'=>false));
        {
            print '<div class="timeline">';
            if ($trace)
            for ($i=0; $i < count($trace); $i++) {
                switch($trace[$i]["gateway_trace_status_id"])
                {
                    case 1:
                        $icon = 'fa fa-plus';
            ...
            ?>
                ...
                <div class="timeline-item">
                    <div class="timeline-badge mt-timeline-icon <?php print $trace[$i]["color"];?> bg-font-red border-grey-steel">
                        <i class="<?php print $trace[$i]["icon"];?>"></i>
                    </div>
                    <div class="timeline-body">
                        <div class="timeline-body-arrow"> </div>
                        <div class="timeline-body-head">
                            <div class="timeline-body-head-caption">
                                <span class="timeline-body-alerttitle font-red-intense"><?php print $trace[$i]["gateway_trace_status"];?></span>
                                <span class="timeline-body-time font-grey-cascade"><?php print $trace[$i]["timestamp"];?></span>
                            </div>
                            <div class="timeline-body-head-actions"> </div>
                        </div>
                        <div class="timeline-body-content">
                            <a href="javascript:;" class="timeline-body-title font-blue-madison"><?php echo $description;?>: <?php print $trace[$i]["description"];?></a>
                            <span class="font-grey-cascade"> (<?php print $trace[$i]["NAME"];?>)</span>
                        </div>
                    </div>
                </div>
                <!-- END TIMELINE ITEM -->
