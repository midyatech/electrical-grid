 ...
 * */
	...
	function utm2ll($x,$y,$zone,$aboveEquator){
		...
		//return array("lat"=>$latlon[0], "long"=>$latlon[1]);
		return array('lat'=>radian2degree($latlon[0]),'long'=>radian2degree($latlon[1]));
	}

	function ll2utm($lat,$lon){
		if($lat != "" && $lon != ""){
			if(!is_numeric(trim($lon))){
				return json_encode(array('success'=>false,'msg'=>"Wrong longitude value ".$lon."ZZZ"));
			}
