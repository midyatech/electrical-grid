...
class Survey
{
    ...
    {
        $params=null;
        $sql="SELECT DISTINCT service_point.point_id, service_point.point_type_id, service_point.single_phase_consumers, service_point.three_phase_consumers, service_point.accuracy_id, service_point.station_id,
                    ...
                    sp2.station_id station_id1, sp2.feeder_id feeder_id1, sp2.capacity_id capacity_id1, sp2.transformer_number transformer_number1,
        ...
        if(isset($condition["station_id"])){
            $condition["sp2.station_id"] = $condition["station_id"];
    ...
    public function GetGatewayServicePoint($condition = null, $order = null, $start = 0, $size = 0, &$recordsCount = null)
    {
        ...
        if(isset($condition["station_id"]) && $condition["station_id"] != "" ){
            $Where = "AND sp2.station_id = ". $this->db->SqlVal($condition["station_id"], "int");
        }
    ...
    public function GetTransformer($condition = null, $order = null, $start = 0, $size = 0, &$recordsCount = null)
    {
        ...
        $params=null;
        $sql="SELECT point_id, point_type_id, single_phase_consumers, three_phase_consumers, accuracy_id, station_id,
                        feeder_id, capacity_id, transformer_number, latitude, longitude, user_id, area_id, timestamp, needs_gateway,
                        CONCAT(service_point.transformer_code, service_point.transformer_gov_number) AS transformer_generated_number
                FROM service_point WHERE point_id NOT IN ($assembly_order_transformers)";
    ...
    public function GetServicePointByArea($condition = null, $order = null, $start = 0, $size = 0, &$recordsCount = null){
        ...
        $sql = "SELECT  DISTINCT service_point.point_id, service_point.point_type_id, single_phase_consumers, three_phase_consumers,
                    accuracy_id, station_id, feeder_id, capacity_id, transformer_number, latitude, longitude, user_id, area_id,
    ...
    public function GetTransformers($area_id=null, $station_id = NULL, $feeder_id, $assoc=1)
    {
        $sql = "SELECT service_point.point_id as transformer_id, CONCAT(transformer_number, ' [' ,service_point.transformer_code, service_point.transformer_gov_number, ']') AS transformer_number,
                    service_point.station_id, service_point.feeder_id, capacity_id, latitude, longitude, user_id, service_point.area_id, timestamp
                FROM service_point
        ...
        if( $station_id != "") {
            $sql .= " AND service_point.station_id = ".$this->db->SqlVal($station_id, "int");
        }
        ...
        //print $sql;
        $result = $this->db->SelectData($sql, NULL, NULL, NULL, NULL, $recordsCount, $assoc);
        $this->Message = $this->db->message;
        $this->State = $this->db->state;
        return $result;
    }
    ...
    {
        $sql = "SELECT feeder.*, station, NODE_NAME as area, station.area_id as area_id
                FROM feeder
                LEFT JOIN station on station.station_id = feeder.station_id
                LEFT JOIN DIR_TREE t ON t.NODE_ID = station.area_id
                WHERE feeder_id=?";
    ...
    {
        $sql = "SELECT service_point.point_id as transformer_id, CONCAT(transformer_number, ' [' ,service_point.transformer_code, service_point.transformer_gov_number, ']') AS transformer_number,
                    service_point.station_id, service_point.feeder_id, capacity_id, latitude, longitude, user_id, service_point.area_id,
    ...
    {
        $sql = "SELECT count(*) FROM feeder WHERE station_id=?";
        $params = $this->db->ConvertToParamsArray([$station_id]);
        $feeders = $this->db->SelectValue($sql, $params);
        if (!$feeders) {
            $condition = array();
            $condition['station_id'] = $this->db->SqlVal($station_id, "int");
            $result = $this->db->Delete("station", $condition);
            $this->Message = $this->db->message;
            return $result;
        } else {
            $this->Message = "station_has_feeders";
            return false;
        }
    }
    ...
    public function DeleteFeeder($feeder_id)
    {
        $sql = "SELECT count(*) FROM service_point WHERE feeder_id=?";
        $params = $this->db->ConvertToParamsArray([$feeder_id]);
        $transformers = $this->db->SelectValue($sql, $params);
        if (!$transformers) {
            $condition = array();
            $condition['feeder_id'] = $this->db->SqlVal($feeder_id, "int");
            $result = $this->db->Delete("feeder", $condition);
            $this->Message = $this->db->message;
            return $result;
        } else {
            $this->Message = "feeder_has_transformers";
            return false;
        }
    }
