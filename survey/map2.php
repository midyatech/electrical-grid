...
<script>
    ...
    function CheckChildren(obj){
        ...
        ids = $("#node_ids").val();
        $('#example').DataTable().ajax.url('api/list.service.point.count.by.area.php?id=' +ids).load();
    }
    ...
    function AddPins(data, map, layerGroup, pin_area){
        for (var i in data) {
            ...
            var accurancy_id = survey["accuracy_id"]+"%";
            ...
            if(lat != null && long != null){
                for(i=0; i<map.polygons.length; i++){
                    if (pin_area == area) {
                        PopUpString = "";
                        if(point_type_id == 4 ){
                            PopUpString = point_type+"<br /><b>"+station_id+"/"+feeder_id+"/"+capacity_id+"/"+transformer_number+"/"+"</b>";
                        } else {
                            PopUpString = point_type+"<br />"+"<?php print $dictionary->GetValue("single_phase_consumers"); ?>"+": <b>"+single_phase_consumers+"</b><br />"+"<?php print $dictionary->GetValue("three_phase_consumers"); ?>"+": <b>"+three_phase_consumers+"</b>";
                        }
