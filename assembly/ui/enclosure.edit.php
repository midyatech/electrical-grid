...
function DrawEnclosure($gateway, $meters_config, $meter_type_id)
{
    ...
    if ($gateway == 1) {
        $tabIndex++;
        print '<div class="col-xs-12 gateway"><div><img src="../img/meters/GW30.png" >
                <input class="form-control" type="text" id="gateway_id" name="gateway_id" placeholder="'.$dictionary->GetValue("Gateway SN").'" tabindex="'.$tabIndex.'" >
                </div></div>';
    }
    ...
    for ($i=0; $i<count($meters); $i++) {
        ...
        if ($meters[$i] == 0) {
            $disabled = true;
            print '<img src="../img/meters/no_item.jpg">';
        } else {
            $disabled = false;
            $tabIndex++;
            print '<img src="../img/meters/'.$img.'.png" >';
            print '<input type="text" id="meter_'.$i.'" name="meter_'.$i.'" '. ($disabled ? 'disabled' : '') .' class="form-control meter-control" tabindex="'.$tabIndex.'" value="">';
        }
        print '</div></div>';
    }
}
        ...
        <div class="well" style="background-color:#fff; border:solid 1px #999">
            <div class="row">
                <div class="col-lg-12">
                    ...
                    $html->DrawFormField("label", "start_date", $enclosure[0]["timestamp"], null, array());

                    $html->DrawFormField("label", "enclosure_type", $enclosure[0]["enclosure_type"], null, array());
        ...
        $('input[type=text]').each(function () {
            ...
            if ($(this).attr("id") != $(control).attr("id")) {
                if ($(this).val() == $(control).val()) {
                    console.log($(this).attr("id") + "-"+$(control).attr("id"))
                    errBeep.play();
                    $(control).focus();
                    $(control).select();
                    return false;
