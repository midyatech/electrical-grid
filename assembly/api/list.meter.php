...
if(isset($_REQUEST["order"])){
    if($_REQUEST["order"][0]["meter_sn"] == "asc"){
        $direction = "asc";
    }else{
        $direction = "desc";
    }
...
if(isset($_REQUEST["from_date"]) && $_REQUEST["from_date"]!=NULL && isset($_REQUEST["to_date"]) && $_REQUEST["to_date"]!=NULL){
    $filter["meter.timestamp#1"] = array("Operator"=>">=","Value"=>$_REQUEST["from_date"]." 00:00:01", "Type"=>"mytext");
    $filter["meter.timestamp#2"] = array("Operator"=>"<", "Value"=>$_REQUEST["to_date"]." 23:59:59", "Type"=>"mytext");
}
...
$AllMeters = $Enclosure->GetMeters($filter, $order, $startingRecord, $pageSize, $totalRecords);
...
echo '"data": ';
if(count($AllMeters) > 0){
    echo json_encode( $AllMeters );
}else{
    echo "{}";
}
echo "}";
